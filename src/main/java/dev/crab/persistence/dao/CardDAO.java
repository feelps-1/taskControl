...
import static dev.crab.persistence.converter.OffSetDateTimeConverter.toOffsetDateTime;
...
public class CardDAO {
    ...
    public void moveToColumn(final Long columnId, Long cardId) throws SQLException{
        ...
        var sql = """
                SELECT c.id,
                       ...
                       (SELECT COUNT(sub_b.card_id)
                                      FROM blocks sub_b WHERE sub_b.card_id = c.id) blocks_amount
                FROM cards c
                LEFT JOIN blocks b
                    ON c.id = b.card_id
